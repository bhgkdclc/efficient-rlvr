...
OUTPUT_PATH="${OUTPUT_PATH:-${REPO_ROOT}/experiments/difficulty_coverage_smoke_${RUN_TAG}}"
...
uv run --no-sync python scripts/train_grpo.py \
    ...
    --run-name "difficulty-coverage-smoke-${RUN_TAG}" \
    --seed 42 \
    --sampling-strategy difficulty \
    --difficulty-ema-beta 0.5 \
    --difficulty-coverage-weight 0.3 \
...
echo "Coverage-aware Difficulty smoke output: ${OUTPUT_PATH}"
