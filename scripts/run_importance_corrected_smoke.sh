...
OUTPUT_PATH="${OUTPUT_PATH:-${REPO_ROOT}/experiments/difficulty_importance_smoke_${RUN_TAG}}"
...
uv run --no-sync python scripts/train_grpo.py \
    ...
    --run-name "difficulty-importance-smoke-${RUN_TAG}" \
    --seed 42 \
    ...
    --importance-weight-clip-min 0.25 \
    --importance-weight-clip-max 4.0 \
...
echo "Importance-corrected smoke output: ${OUTPUT_PATH}"
