...
OUTPUT_PATH="${OUTPUT_PATH:-${REPO_ROOT}/experiments/annealed_difficulty_smoke_${RUN_TAG}}"
...
uv run --no-sync python scripts/train_grpo.py \
    ...
    --run-name "annealed-difficulty-smoke-${RUN_TAG}" \
    --seed 42 \
    --sampling-strategy difficulty_then_random \
    --difficulty-switch-rollout-tokens 1 \
...
echo "Annealed difficulty smoke output: ${OUTPUT_PATH}"
