...
OUTPUT_PATH="${OUTPUT_PATH:-${REPO_ROOT}/experiments/difficulty_smoke_${RUN_TAG}}"
...
uv run --no-sync python scripts/train_grpo.py \
    ...
    --run-name "difficulty-smoke-${RUN_TAG}" \
    --seed 42 \
...
echo "Difficulty-aware smoke output: ${OUTPUT_PATH}"
